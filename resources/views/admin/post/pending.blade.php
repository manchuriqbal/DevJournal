                        ...
                        <tr>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                @if (auth()->guard('admin')->check())
                                    {{ $post->postable ? $post->postable->name : 'Unknown Admin' }}
                                @elseif (auth()->guard('creator')->check())
                                    {{ $post->postable ? $post->postable->name : 'Unknown Creator' }}
                                @else
                                    Unknown
                            ...
                            <td class="flex items-center gap-2 px-6 py-4 w-48 text-sm text-gray-500">
                                <form action="{{ route('admin.manchur', $post->id) }}" method="POST">
                                    @csrf
                                    @method('PATCH')
                                    ...
                                    <select name="status" required
                                        ...
                                        @foreach ($statusValues as $status)
                                            <option value="{{ $status }}" @selected($post->status == $status)>
                                                {{ ucfirst($status) }}
                                            </option>
                                        @endforeach
                                    </select>
                                    ...
                                    <button type="submit"
                                        class="mt-1 inline-flex items-center px-2 py-1 text-xs font-medium text-white bg-blue-600 rounded-md hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-1 focus:ring-blue-500 shadow">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                <div class="flex justify-center space-x-2">
                                    ...
                                    @if ($post->postable_type == 'App\Models\Admin' && $post->postable_id == auth()->guard('admin')->id())
