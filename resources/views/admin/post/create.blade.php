...
@section('content')
    <div class="bg-white p-6 rounded-lg shadow-md mb-8 max-w-3xl mx-auto">
        ...
        <form action="{{ route('admin.posts.store') }}" method="POST" class="space-y-6" enctype="multipart/form-data">
            ...
            <!-- Author -->
            <input type="hidden" name="postable_type"
                value="{{ auth()->guard('admin')->check() ? 'App\\Models\\Admin' : 'App\\Models\\Creator' }}">
            <input type="hidden" name="postable_id"
                value="{{ auth()->guard('admin')->check() ? auth()->guard('admin')->id() : auth()->guard('creator')->id() }}">
            {{-- <div>
